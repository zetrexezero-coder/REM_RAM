...
if is_module_loaded(FILENAME):
    ...
    def disable(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            ...
            update.effective_message.reply_text("ماذا تريدني أن أعطّل؟")
    ...
    def disable_module(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            ...
            try:
                ...
                update.effective_message.reply_text("هل تلك الوحدة موجودة أصلاً؟")
                return
    ...
    def enable(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            ...
            if sql.enable_command(chat.id, enable_cmd):
                ...
                update.effective_message.reply_text("هل هذا معطّل أصلاً؟")

        else:
            update.effective_message.reply_text("ماذا تريدني أن أفعّل؟")
    ...
    def list_cmds(update: Update, context: CallbackContext):
        if DISABLE_CMDS + DISABLE_OTHER:
            ...
            update.effective_message.reply_text("لا يمكن تعطيل أي أمر.")
    ...
    __help__ = """
      • `/cmds`*:* التحقق من الحالة الحالية للأوامر المعطّلة

      *للمشرفين فقط:*
      • `/enable <اسم الأمر>`*:* تفعيل ذلك الأمر
      • `/disable <اسم الأمر>`*:* تعطيل ذلك الأمر
      • `/enablemodule <اسم القسم>`*:* تفعيل جميع أوامر ذلك القسم
      • `/disablemodule <اسم القسم>`*:* تعطيل جميع أوامر ذلك القسم
      • `/listcmds`*:* عرض جميع الأوامر القابلة للتبديل
      """

    __mod_name__ = "تعطيل الأوامر"
