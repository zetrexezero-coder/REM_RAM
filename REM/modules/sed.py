...
__help__ = """
   • `s/<النص1>/<النص2>(/<العلامة>)`*:* رد على رسالة بهذا لتنفيذ عملية استبدال نصي، تستبدل جميع   تكرارات 'النص1' بـ 'النص2'. العلامات اختيارية، وتشمل حالياً 'i' لتجاهل حالة الأحرف، 'g' للاستبدال الشامل،   أو بدون أي علامة. الفواصل المتاحة هي `/`، `_`، `|`، و`:`. يدعم التجميع النصي. لا يمكن أن تكون الرسالة الناتجة   أكبر من {}.
  *تذكير:* الاستبدال النصي يستخدم بعض الرموز الخاصة لتسهيل المطابقة، مثل: `+*.?\`
  إذا أردت استخدام هذه الرموز، تأكد من الهروب منها!
  *مثال:* \?.
  """.format(
    telegram.MAX_MESSAGE_LENGTH,
)

__mod_name__ = "الاستبدال النصي"
