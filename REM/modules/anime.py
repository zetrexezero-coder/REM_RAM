...
def anime(update: Update, context: CallbackContext):
    ...
    if "errors" in json.keys():
        update.effective_message.reply_text("الأنمي غير موجود")
        return
    if json:
        ...
        if trailer:
            buttons = [
                [
                    InlineKeyboardButton("معلومات أكثر", url=info),
                    InlineKeyboardButton("الإعلان 🎬", url=trailer),
                ],
            ]
        else:
            buttons = [[InlineKeyboardButton("معلومات أكثر", url=info)]]
        if image:
            try:
...
def character(update: Update, context: CallbackContext):
    ...
    if "errors" in json.keys():
        update.effective_message.reply_text("الشخصية غير موجودة")
        return
    if json:
...
def manga(update: Update, context: CallbackContext):
    ...
    msg = ""
    if "errors" in json.keys():
        update.effective_message.reply_text("المانجا غير موجودة")
        return
    if json:
        json = json["data"]["Media"]
...
def user(update: Update, context: CallbackContext):
    ...
    try:
        ...
        update.effective_message.reply_text("اسم المستخدم غير موجود.")
        return

    progress_message = update.effective_message.reply_text("جارٍ البحث.... ")
...
def site_search(update: Update, context: CallbackContext, site: str):
    ...
    if not search_query:
        message.reply_text("أعطني شيئاً للبحث عنه")
        return
    ...
    buttons = [[InlineKeyboardButton("عرض جميع النتائج", url=search_url)]]
...
__help__ = """
  احصل على معلومات عن الأنمي أو المانجا أو الشخصيات من [AniList](anilist.co).

  *الأوامر المتوفرة:*

   • `/anime <اسم الأنمي>`*:* يعرض معلومات عن الأنمي.
   • `/character <اسم الشخصية>`*:* يعرض معلومات عن الشخصية.
   • `/manga <اسم المانجا>`*:* يعرض معلومات عن المانجا.
   • `/user <المستخدم>`*:* يعرض معلومات عن مستخدم في MyAnimeList.
   • `/upcoming`*:* يعرض قائمة الأنميات القادمة في المواسم المقبلة.
   • `/kaizoku <اسم الأنمي>`*:* بحث عن أنمي في animekaizoku.com
   • `/kayo <اسم الأنمي>`*:* بحث عن أنمي في animekayo.com
   • `/airing <اسم الأنمي>`*:* يعرض معلومات بث الحلقات.
