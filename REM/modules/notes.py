...
def slash_get(update: Update, context: CallbackContext):
    ...
    try:
        ...
        update.effective_message.reply_text("معرّف الملاحظة غير صحيح 😾")
...
def clear(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if sql.rm_note(chat_id, notename):
            update.effective_message.reply_text("تم حذف الملاحظة بنجاح.")
        else:
    ...
    if member.status != "creator" and user.id not in DRAGONS:
        ...
        buttons = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        text="حذف جميع الملاحظات", callback_data="notes_rmall",
                    ),
                ],
...
def list_notes(update: Update, context: CallbackContext):
    ...
    if not note_list:
        try:
            update.effective_message.reply_text("لا توجد ملاحظات في هذه المجموعة!")
        except BadRequest:
            update.effective_message.reply_text("لا توجد ملاحظات في هذه المجموعة!", quote=False)
...
__help__ = """
   • `/get <اسم الملاحظة>`*:* الحصول على الملاحظة بهذا الاسم
   • `#<اسم الملاحظة>`*:* نفس /get
   • `/notes` أو `/saved`*:* عرض جميع الملاحظات المحفوظة في هذا القروب
   • `/number` *:* يعرض الملاحظة برقمها في القائمة
  إذا أردت استرجاع محتوى ملاحظة بدون أي تنسيق، استخدم `/get <اسم الملاحظة> noformat`. قد يكون   هذا مفيداً عند تحديث ملاحظة حالية

  *للمشرفين فقط:*
   • `/save <اسم الملاحظة> <المحتوى>`*:* حفظ المحتوى كملاحظة باسم اسم الملاحظة
  يمكن إضافة زر للملاحظة باستخدام صيغة روابط الماركداون العادية - يجب أن يسبق الرابط بقسم   `buttonurl:`، مثل: `[رابط](buttonurl:example.com)`. راجع `/markdownhelp` لمزيد من المعلومات
   • `/save <اسم الملاحظة>`*:* حفظ الرسالة المردود عليها كملاحظة باسم اسم الملاحظة
   اجعل الردود المختلفة منفصلة بـ `%%%` للحصول على ملاحظات عشوائية
   *مثال:*
   `/save اسم_الملاحظة
   رد 1
   %%%
   ...
   %%%
   رد 3`
   • `/clear <اسم الملاحظة>`*:* حذف ملاحظة بهذا الاسم
   • `/removeallnotes`*:* حذف جميع ملاحظات القروب
   *ملاحظة:* أسماء الملاحظات لا تفرّق بين الأحرف الكبيرة والصغيرة، ويتم تحويلها تلقائياً لأحرف صغيرة عند الحفظ.
  """
