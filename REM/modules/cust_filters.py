...
def filters(update, context):
    ...
    if not msg.reply_to_message and len(extracted) >= 2:
        ...
        send_message(update.effective_message, "فلتر غير صحيح!")
        return
...
def stop_filter(update, context):
    ...
    if len(args) < 2:
        send_message(update.effective_message, "ماذا تريدني أن أوقف؟")
        return
            ...
            [
                [
                    InlineKeyboardButton(
                        text="إيقاف جميع الفلاتر", callback_data="filters_rmall",
                    ),
                ],
...
def addnew_filter(update, chat_id, keyword, text, file_type, file_id, buttons):
    ...
    if len(totalfilt) >= 150:  # Idk why i made this like function....
        msg.reply_text("وصلت المجموعة للحد الأقصى من الفلاتر (150).")
        return False
    else:
...
__help__ = """
   • `/filters`*:* عرض جميع الفلاتر النشطة المحفوظة في القروب.

  *للمشرفين فقط:*
   • `/filter <الكلمة> <الرد>`*:* إضافة فلتر لهذا القروب. سيرد البوت بتلك الرسالة عند ذكر 'الكلمة'  . ملاحظة: كل كلمات الفلاتر تكون بأحرف صغيرة. إذا أردت أن تكون الكلمة جملة، استخدم علامات التنصيص. مثال: /filter "hey there" How you   doin?
   اجعل الردود المختلفة منفصلة بـ `%%%` للحصول على ردود عشوائية
   *مثال:*
   `/filter "filtername"
   رد 1
   %%%
  ...
  *لمنشئ القروب فقط:*
   • `/removeallfilters`*:* إزالة جميع فلاتر القروب دفعة واحدة.

  *ملاحظة*: الفلاتر تدعم أيضاً منسقات الماركداون مثل: {first}، {last} وغيرها.. والأزرار.
  راجع `/markdownhelp` لمعرفة المزيد!
  """
