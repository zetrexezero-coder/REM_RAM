...
if is_module_loaded(FILENAME):
    ...
    def logging(update: Update, context: CallbackContext):
        ...
        if log_channel:
            ...
            message.reply_text("لم يتم ضبط قناة سجل لهذه المجموعة!")
        ...
        if chat.type == chat.CHANNEL:
            ...
            try:
                ...
                if excp.message == "Forbidden: bot is not a member of the channel chat":
                    bot.send_message(chat.id, "تم ضبط قناة السجل بنجاح!")
                else:
    ...
    def unsetlog(update: Update, context: CallbackContext):
        ...
        if log_channel:
            ...
            message.reply_text("تم إلغاء قناة السجل.")

        else:
            message.reply_text("لم يتم ضبط قناة سجل بعد!")
    ...
    __help__ = """
  *للمشرفين فقط:*
  • `/logchannel`*:* عرض معلومات قناة السجلات
  • `/setlog`*:* تعيين قناة السجلات.
  • `/unsetlog`*:* إلغاء تعيين قناة السجلات.

  يتم تعيين قناة السجلات عبر:
  • إضافة البوت للقناة المطلوبة (كمشرف!)
  • إرسال `/setlog` في القناة
  • إعادة توجيه `/setlog` إلى القروب
  """

    __mod_name__ = "قناة السجلات"
