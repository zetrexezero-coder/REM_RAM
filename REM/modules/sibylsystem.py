    ...
    if sibylClient:
        ...
        for user in users:
            ...
            if data.banned:
                ...
                keyboard = [[InlineKeyboardButton(text="معلومات أكثر", url=url)]]
...
def handle_sibyl_banned(user, data):
    ...
    keyboard = [[InlineKeyboardButton("استئناف", url="https://t.me/SibylRobot")]]
    reply_markup = InlineKeyboardMarkup(keyboard)

    try:
        bot.send_message(user.id, "تمت إضافتك إلى قاعدة بيانات سيبل", parse_mode=ParseMode.HTML, reply_markup=reply_markup)
    except BaseException as e:
        logging.error(e)
...
modes_txt = '''
أوضاع نظام سيبل:
 • <b>تفاعلي</b> - مكافحة الإسبام مع تنبيهات
 • <b>صامت</b> - معالجة المستخدمين السيئين في الخلفية بصمت
 • <b>تنبيهات فقط</b> - تنبيهات فقط عن المستخدمين السيئين، بدون إجراءات

إعدادات إضافية:
 • <b>قناة السجل</b> - ينشئ سجلاً في قناة السجل (إن وُجدت) لجميع أحداث سيبل

الإعدادات الحالية:'''

connection_txt = '''
يمكن تشغيل وإيقاف الاتصال بـ <a href="https://t.me/SibylSystem/2">نظام سيبل</a> من خلال أزرار اللوحة أدناه.
'''
...
def sibylmain(update: Update, _: CallbackContext):
    ...
    if update.callback_query:
        if update.callback_query.data == "sibyl_connect=toggle":
            ...
            if stat:
                ...
                stat = True
            update.callback_query.answer(f'تم {"تفعيل" if stat else "تعطيل"} نظام سيبل!')
    ...
    text = f'{connection_txt}\n • <b>الحالة الحالية:</b> <code>{"مفعّل" if stat else "معطّل"}</code>'
    keyboard = [
        [
            InlineKeyboardButton(
                    "✤ قطع الاتصال" if stat else "✤ Connect",
...
def sibyltoggle(update: Update, _: CallbackContext):
    ...
    if not is_user_admin(chat, user.id):
        update.callback_query.answer("المشرفون فقط يمكنهم تغيير هذا!")
        return
    ...
    text = f'{modes_txt}\n • <b>الوضع:</b> <code>{SibylMode(act).name}</code>\n'
    text += f' • <b>السجلات:</b> <code>{"مفعّلة" if log_stat else "معطّلة"}</code>'
    keyboard = [
        [
            InlineKeyboardButton(
        ...
        [
            ...
            InlineKeyboardButton(
                    "تعطيل السجل" if log_stat else "تفعيل السجل",
...
def get_sibyl_info(bot: Bot, user: User, detailed: bool = False) -> (str, Optional[InlineKeyboardMarkup]):
    reply_markup = None
    txt = "<b>نتائج الفحص السيمي</b>"
    txt += f"\n • <b>المستخدم</b>: {mention_html(user.id, user.first_name)}"
    txt += f"\n • <b>المعرّف</b>: <code>{user.id}</code>"
    ...
    if data:
        txt += f"\n • <b>محظور:</b> <code>{'لا' if not data.banned else 'نعم'}</code>"
        cc = data.crime_coefficient or"?"
        txt += f"\n • <b>معامل الجريمة:</b> <code>{cc}</code> [<a href='https://t.me/SibylSystem/3'>?</a>]"
        hue = data.hue_color or "?"
        txt += f"\n • <b>لون Hue:</b> <code>{hue}</code> [<a href='https://t.me/SibylSystem/5'>?</a>]"
        if data.ban_flags:
            txt += f"\n • <b>سبب العلامة:</b> <code>{', '.join(data.ban_flags)}</code>"
        if data.date:
            txt += f"\n • <b>التاريخ:</b> <code>{data.date}</code>"
        if data.is_bot:
            txt += "\n • <b>بوت:</b> <code>نعم</code>"

        if data.crime_coefficient < 10:
            txt += "\n • <b>الحالة:</b> <code>مفتش</code>"
        elif 10 <= data.crime_coefficient < 80:
            txt += "\n • <b>الحالة:</b> <code>مدني</code>"
        elif 81 <= data.crime_coefficient <= 100:
            txt += "\n • <b>الحالة:</b> <code>مُعاد تأهيله</code>"
        elif 101 <= data.crime_coefficient <= 150:
            txt += "\n • <b>الحالة:</b> <code>مُنفّذ</code>"

        if detailed:
            if data.reason:
                txt += f"\n • <b>السبب:</b> <code>{data.reason}</code>"
            if data.ban_source_url:
                txt += f"\n • <b>المصدر:</b> <a href='{data.ban_source_url}'>رابط</a> "
            if data.source_group:
                txt += f"\n • <b>المصدر المرفق:</b> <code>{data.source_group}</code>"
            if data.message:
                txt += f"\n • <b>رسالة الحظر:</b> {data.message}"

    else:
        txt += "\n • <b>محظور:</b> <code>لا</code>"
        txt += f"\n • <b>معامل الجريمة:</b> <code>?</code> [<a href='https://t.me/SibylSystem/3'>?</a>]"
        txt += f"\n • <b>لون Hue:</b> <code>?</code> [<a href='https://t.me/SibylSystem/5'>?</a>]"

    txt += "\n\nمدعوم من @SibylSystem | @Kaizoku"
    if data and data.banned:
        keyboard = [[]]
        if not detailed:
            url = helpers.create_deep_linked_url(bot.username, f"sibyl_banned-{user.id}")
            keyboard[0].append(InlineKeyboardButton("معلومات أكثر", url = url))
        keyboard[0].append(InlineKeyboardButton("استئناف", url = "https://t.me/SibylRobot"))
        reply_markup = InlineKeyboardMarkup(keyboard)
...
__help__ = """
  [نظام سيبل](https://t.me/SibylSystem/14) هو قسم مكافحة إسبام مستوحى من أنمي "[PsychoPass]". 
  هذا القسم قادر على التعامل تفاعلياً أو بصمت مع المستخدمين السيئين الذين تعرف عليهم سيبل بأنهم ضارّون.

  القسم مفعّل بشكل افتراضي ويأتي بأمرين. 

  *الأوامر المتوفرة:* 
   • `/sibyl`*:* شغّل هذا في قروب للتحكم بالإعدادات
   • `/check`*:* أمر معلومات للتحقق من وجود مستخدم في قاعدة بيانات سيبل
   
  مصطلحات أخرى 
  • [معامل الجريمة](https://t.me/SibylSystem/3)
  • [علامات وأسباب الحظر](https://t.me/SibylSystem/4)
  • [شرح ألوان Hue](https://t.me/SibylSystem/5) 
  • [مساعدة ووثائق API](https://t.me/PsychoPass/5)
  • [قروب الدعم](https://t.me/PublicSafetyBureau)
  • [تبليغ المستخدمين السيئين](https://t.me/MinistryOfWelfare/8)
  """


__mod_name__ = "نظام سيبل"
