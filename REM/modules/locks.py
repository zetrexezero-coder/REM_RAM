        ...
        if len(args) >= 1:
            ...
            if ltype in LOCK_TYPES:
                ...
                if conn:
                    ...
                    text = "تم قفل {} للغير مشرفين في {}!".format(ltype, chat_name)
                else:
                    ...
                    text = "تم قفل {} للغير مشرفين!".format(ltype)
                sql.update_lock(chat.id, ltype, locked=True)
                # تحذير: إذا البوت ما عنده صلاحية الحذف، القفل ما يطبّق
                if not can_delete(chat, context.bot.id):
                    text += "\n⚠️ تحذير: ما عندي صلاحية حذف الرسائل، القفل لن يطبّق حتى تعطيني صلاحية حذف رسائل الأعضاء!"
                ...
                if conn:
                    ...
                    chat_id = conn
                    chat_name = chat.title
                    text = "تم قفل {} لجميع غير المشرفين في {}!".format(
                        ltype, chat_name,
                    )
                else:
                    if update.effective_message.chat.type == "private":
                    ...
                    chat_id = update.effective_chat.id
                    chat_name = update.effective_message.chat.title
                    text = "تم قفل {} لجميع غير المشرفين!".format(ltype)
                ...
                send_message(
                    update.effective_message,
                    "ماذا تحاول قفله؟ جرب /locktypes لعرض قائمة ما يمكن قفله",
                )
        else:
            send_message(update.effective_message, "ماذا تحاول قفله؟")

    else:
        send_message(
            update.effective_message,
            "أنا مو مشرف أو ما عندي صلاحيات كافية.",
        )
        ...
        if len(args) >= 1:
            ...
            if ltype in LOCK_TYPES:
                ...
                if conn:
                    ...
                    chat_id = conn
                    chat_name = chat.title
                    text = "تم فتح قفل {} للجميع في {}!".format(ltype, chat_name)
                else:
                    if update.effective_message.chat.type == "private":
                        send_message(
                            update.effective_message,
                    ...
                    chat_id = update.effective_chat.id
                    chat_name = update.effective_message.chat.title
                    text = "تم فتح قفل {} للجميع!".format(ltype)
                ...
                if not can_change_info:
                    send_message(
                        update.effective_message,
                        "ما عندي صلاحية تغيير معلومات القروب.",
                        parse_mode="markdown",
                    )
                    return
                ...
                send_message(
                    update.effective_message,
                    "ماذا تحاول فتح قفله؟ جرب /locktypes لعرض قائمة ما يمكن قفله.",
                )

        else:
            send_message(update.effective_message, "ماذا تحاول فتح قفله؟")
                ...
                for new_mem in new_members:
                    if new_mem.is_bot:
                        if not is_bot_admin(chat, context.bot.id):
                            send_message(
                                update.effective_message,
                                "شايف بوت وعندي أوامر أمنعه من الدخول... بس أنا مو مشرف!",
                            )
                            return
...
__help__ = """
  هل تضيق ذرعاً بالملصقات؟ أو تريد منع مشاركة الروابط؟ أو الصور؟   أنت في المكان الصحيح!
  قسم الأقفال يتيح لك قفل بعض العناصر الشائعة في   عالم تلغرام؛ سيحذفها البوت تلقائياً!

   • `/locktypes`*:* عرض جميع أنواع الأقفال الممكنة

  *للمشرفين فقط:*
   • `/lock <النوع>`*:* قفل عناصر من نوع معين (غير متوفر في الخاص)
   • `/unlock <النوع>`*:* فتح قفل عناصر من نوع معين (غير متوفر في الخاص)
   • `/locks`*:* قائمة الأقفال الحالية في هذا القروب.

  يمكن استخدام الأقفال لتقييد أعضاء القروب.
  مثال:
  قفل الروابط سيحذف تلقائياً كل الرسائل التي تحتوي روابط، وقفل الملصقات سيمنع   جميع الأعضاء غير المشرفين من إرسال الملصقات، وغيره.
  قفل البوتات سيمنع غير المشرفين من إضافة بوتات للقروب.

  *ملاحظة:*
   • فتح صلاحية *info* سيسمح للأعضاء (غير المشرفين) بتغيير معلومات القروب، مثل الوصف أو اسم القروب
   • فتح صلاحية *pin* سيسمح للأعضاء (غير المشرفين) بتثبيت رسالة في القروب
  """
