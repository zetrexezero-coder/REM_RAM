...
def load(update: Update, context: CallbackContext):
    ...
    load_messasge = message.reply_text(
        f"جارٍ محاولة تحميل الوحدة: <b>{text}</b>", parse_mode=ParseMode.HTML,
    )

    try:
        imported_module = importlib.import_module("SaitamaRobot.modules." + text)
    except:
        load_messasge.edit_text("هل هذه الوحدة موجودة أصلاً؟")
        return
    ...
    if imported_module.__mod_name__.lower() not in IMPORTED:
        ...
        load_messasge.edit_text("الوحدة محملة بالفعل.")
        return
    if "__handlers__" in dir(imported_module):
        ...
        load_messasge.edit_text("لا يمكن تحميل هذه الوحدة.")
        return
    ...
    load_messasge.edit_text(
        "تم تحميل الوحدة بنجاح: <b>{}</b>".format(text), parse_mode=ParseMode.HTML,
    )
...
def unload(update: Update, context: CallbackContext):
    ...
    unload_messasge = message.reply_text(
        f"جارٍ محاولة إلغاء تحميل الوحدة: <b>{text}</b>", parse_mode=ParseMode.HTML,
    )

    try:
        imported_module = importlib.import_module("SaitamaRobot.modules." + text)
    except:
        unload_messasge.edit_text("هل هذه الوحدة موجودة أصلاً؟")
        return
    ...
    if imported_module.__mod_name__.lower() in IMPORTED:
        ...
        unload_messasge.edit_text("لا يمكن إلغاء تحميل شيء لم يُحمَّل.")
        return
    if "__handlers__" in dir(imported_module):
        handlers = imported_module.__handlers__
        for handler in handlers:
            if isinstance(handler, bool):
                unload_messasge.edit_text("لا يمكن إلغاء تحميل هذه الوحدة!")
                return
    ...
    unload_messasge.edit_text(
        f"تم إلغاء تحميل الوحدة بنجاح: <b>{text}</b>", parse_mode=ParseMode.HTML,
    )
...
def listmodules(update: Update, context: CallbackContext):
    ...
    module_list = "الوحدات المحملة حالياً: \n\n" + "".join(module_list)
